...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $kelasOmsetFiltered = $allProducts->map(function ($kelas) use ($csId, $tahun, $bulanNum) {
            ...
            $omsetSalesplan = $kelas->salesplans()
            ...
            $omsetHistory = \App\Models\HistoryPenjualan::where('user_id', $csId)
                ->where('kelas_id', $kelas->id)
                ->where('bulan', $bulanNum)
                ->where('tahun', $tahun)
                ->sum('omset');

            $omset = $omsetSalesplan + $omsetHistory;
        ...
        // ====================== HISTORY PENJUALAN MANUAL ======================
        $historyPenjualans = \App\Models\HistoryPenjualan::with('kelas')
            ->where('user_id', $csId)
            ->orderBy('tahun', 'desc')
            ->orderBy('bulan', 'desc')
            ->get();
    ...
    private function hitungTotalNilaiHasil($csId, $namaUserData, $bulan, $tahun, $role)
    {
        ...
        foreach ($allProducts as $kelas) {
            ...
            $totalOmset += ($omsetSalesplan + $omsetHistory);
        }
